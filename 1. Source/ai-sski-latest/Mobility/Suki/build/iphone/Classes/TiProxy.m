...
//Rare exceptions to indicate a bug in the _suki code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_suki = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_suki]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.suki.module"

...
namespace Suki
{
    public partial class GRPOList : System.Web.UI.Page
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        protected void lnkPONo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                LinkButton lblGRNNo = (LinkButton)gr.FindControl("lnkPONo");
                Response.Redirect("ReceiveGoods.aspx?IsNew=E&GRPODocEntry=" + lblDocNum.Text + "&GRNNo=" + lblGRNNo.Text + "&Company=" + this.ddlCompany.SelectedValue.ToString());
            }
            catch (Exception ex)
            {
                this.lblError.Text = ex.Message;
        ...
        protected void grvSearchResult_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName.ToUpper() == "COPY")
                {
                    ...
                    Response.Redirect("ReceiveGoods.aspx?docEntry=" + lblDocNum.Text.Trim() + "&Company=" + this.ddlCompany.SelectedValue.ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                Session["CompanyHistoryPOGRPO"] = this.ddlCompany.SelectedValue.ToString();
                Session["OutletPOGRPO"] = this.drpOrderWareHouse.SelectedValue.ToString();
        ...
        private void Search()
        {
            try
            {
                //if (Session[Utils.AppConstants.CompanyCode].ToString() != System.Configuration.ConfigurationManager.AppSettings["HoldingDatabase"].ToString())
                {
                    ...
                    if (Session["CompanyHistoryPOGRPO"] != null)
                    {
                        company = Session["CompanyHistoryPOGRPO"].ToString();
                        this.ddlCompany.SelectedValue = company;
                        LoadOrderWareHouse(company);
                        this.drpOrderWareHouse.SelectedValue = Session["OutletPOGRPO"].ToString();
                    }
                    DataSet ds = obj.GRPOSearch(Session[Utils.AppConstants.UserCode].ToString(), Session[Utils.AppConstants.Pwd].ToString(),
                                        company, fromDate, toDate,
                                          this.txtVendorCode.Text, this.txtGNRNo.Text.Trim(), this.drpOrderWareHouse.SelectedValue.ToString());
                    ...
                    if (ds != null && ds.Tables.Count > 0)
                    {
                        ...
                        foreach (DataRow r in ds.Tables[0].Rows)
                        {
                            ...
                            r["No"] = i;
                            i++;
                            if (Session[Utils.AppConstants.IsCompanySuperUser].ToString().ToUpper() == "N" && dicOutlet.Count > 0 && dicOutlet.ContainsKey(r["U_AB_POWhsCode"].ToString()))
                            {
            ...
            for (int i = 0; i < this.grvSearchResult.Rows.Count; i++)
            {
                ...
                if (checkBox != null)
                {
                    if (checkBox.Checked)
                    {
                        ...
                        string errMsg = Utils.Emails.SendEmail("hieuhm@abeo-electra.com", new string[] { email.Value }, "TEST", "TEST", null, null, null, null);

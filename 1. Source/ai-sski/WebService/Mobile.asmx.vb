...
Public Class Mobile
    ...
    Public Function DriverLogin(ByVal userId As String, ByVal password As String) As String
        Try
    ...
    Public Function Login(ByVal userId As String, ByVal password As String, ByVal dbName As String) As Integer
        Try
            ...
            If errMsg.Length = 0 Then
                ...
                'Return True 'New JavaScriptSerializer().Serialize(True)
                'Return True
            Else
 ...
 <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    ...
    <WebMethod()> _
   <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    Public Function GetSummaryOrder(ByVal driverNo As String) As String
    ...
    <WebMethod()> _
   <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    Public Function GetWorkingOrder(ByVal numRecord As Integer, ByVal OutletCode As String, ByVal driverNo As String) As String
        ...
        For Each dr As DataRow In dtDBName.Rows
            Dim dbName As String = dr("U_DBName")
            sqlStr += " select distinct top (@Param1) do.DocEntry, isnull(m1.BeginStr, '') + CAST(do.DocNum as nvarchar(50)) DocNum, do.CardCode, do.Address2, do.DocDueDate, do.DocDate, fd.Descr, U_AB_DriverNo, do.NumAtCard, '" + dbName + "' U_DBName "
    ...
    <WebMethod()> _
   <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    Public Function GetAllOrder(ByVal OutletCode As String, ByVal driverNo As String) As String
    ...
    <WebMethod()> _
  <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    Public Function GetCompletedOrder(ByVal driverNo As String) As String
        Dim _date As Date = Date.Now
        Dim strDate = _date.ToString("yyyy-MM-dd")
        Dim prev3Days As Date = _date.AddDays(-7)
        Dim strPrev3Day = prev3Days.ToString("yyyy-MM-dd")
    ...
    <WebMethod()> _
   <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    Public Function GetOrderDetail(ByVal DocEntry As Integer, ByVal DBName As String) As String
        'get connect from outlet code

        Dim sisConnect As Connection = getConnectionByDBName(DBName)
        'connect.ConnectDB()
        Dim list As New List(Of DLN1)()
        Dim sqlStr = "select dt.DocEntry, dt.LineNum, dt.ItemCode, dt.Dscription, dt.Quantity, isnull(do.RecQty, 0) U_RecQty, dt.WhsCode, it.ManbtchNum, dt.UnitMsr, dt.U_AB_POQty from DLN1 dt "
  ...
  <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    ...
    <WebMethod()> _
   <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    Public Function PostOrderDetail(ByVal jsonText As String, ByVal DBName As String) As Boolean
   ...
   <ScriptMethod(ResponseFormat:=ResponseFormat.Json)> _
    ...
    <WebMethod()> _
    Public Function Acknowledgment(ByVal DocEntry As Integer, ByVal _username As String, ByVal _password As String, ByVal _outlet As String, ByVal DBName As String, ByVal GRPODate As String) As String
        Dim sFunName As String = String.Empty
        Try
            Dim docDate As Date = Date.Parse(GRPODate)

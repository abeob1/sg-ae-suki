...
Module modCommon
    ...
    Public Function GetSystemIntializeInfo_OLD(ByRef oCompDef As CompanyDefault, ByRef sErrDesc As String) As Long
        ...
        Try
            ...
            ''oCompDef.ServerName = String.Empty
            ''oCompDef.DBName = String.Empty
            ''oCompDef.DBUsername = String.Empty
            ''oCompDef.DBPass = String.Empty
            oCompDef.TradingCompany = String.Empty
            oCompDef.TradingVendor = String.Empty

            '' oCompDef.ServerName = "192.168.100.246" ' "JOHN\SQL2012" '   '''' SUKI

            ''oCompDef.ServerName = p_oHoldingCompany.Server '"AMAZONA-S5ID9AF\SQL2008R2" '   '''' AWS
            oCompDef.DBName = sHoldingDBName '"UAT_HSukiGroup"
            ''oCompDef.DBUsername = p_oHoldingCompany.DbUserName  '"sa"
            ''oCompDef.DBPass = "2012" ' p_oHoldingCompany.DbPassword  '"Sa12345" ' '''' SUKI
            ' oCompDef.DBPass = "Sa12345" ' "2012" ' '''' AWS
            oCompDef.CustomerGroup = "105"
            oCompDef.VendorGroup = "104"
            oCompDef.ItemGroup = "104"

            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            GetSystemIntializeInfo_OLD = RTN_SUCCESS

        Catch ex As Exception
            WriteToLogFile(ex.Message, sFuncName)
            Console.WriteLine("Completed with ERROR ", sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
            GetSystemIntializeInfo_OLD = RTN_ERROR
        End Try
    End Function
    ...
    Public Function GetCompanyDetails_OLD(ByRef oCompany As SAPbobsCOM.Company, sErrDesc As String) As Long
        ...
        Try
            ...
            sQuery = "SELECT * FROM [dbo].[@AB_COMPANYSETUP]"

            da = New SqlDataAdapter(sQuery, objConn)
            oCompanydetailsDT = New DataTable

            da.Fill(oCompanydetailsDT)
            oCompanydetailsDV = New DataView(oCompanydetailsDT)
            oCompanydetailsDV.RowFilter = "U_DBName = '" & oCompany.CompanyDB & "'"
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed With SUCCESS", sFuncName)
            GetCompanyDetails_OLD = RTN_SUCCESS
        Catch ex As Exception
            GetCompanyDetails_OLD = RTN_ERROR
            p_oSBOApplication.StatusBar.SetText(ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed With ERROR  " & ex.Message, sFuncName)
            Call WriteToLogFile(ex.Message, sFuncName)
        End Try
    ...
    Public Function ConnectToTradingCompany(ByRef oCompany As SAPbobsCOM.Company, _
        ...
        Try
            ...
            Else
                ...
                End Select
                'UAT_RIshiMura;manager;1234;JOHN\SQL2012;sa;2012;JOHN;2012
                'RGSRestaurant;manager;1234;192.168.100.7;sa;Sa12345;192.168.100.7:30000;2008
                oCompany.LicenseServer = sTradingCompanyCredentials(6) ' & ":30000"  ' suki 6 john 6
